...
public :
        ...
        FromPV(IsFromPV)
    {
        ...
        if (abs(HadronFlavour) == 5) {
            SFReshaping = readerReshaping.eval_auto_bounds("central", BTagEntry::FLAV_B, abs(Eta), Pt, (Bprob + BBprob));
            SFReshaping_up = readerReshaping.eval_auto_bounds("up_hfstats2", BTagEntry::FLAV_B, abs(Eta), Pt, (Bprob + BBprob));
            SFReshaping_down = readerReshaping.eval_auto_bounds("down_hfstats2", BTagEntry::FLAV_B, abs(Eta), Pt, (Bprob + BBprob));
            ...
            SFReshaping = readerReshaping.eval_auto_bounds("central", BTagEntry::FLAV_C, abs(Eta), Pt, (Bprob + BBprob));
            SFReshaping_up = readerReshaping.eval_auto_bounds("up_hfstats2", BTagEntry::FLAV_C, abs(Eta), Pt, (Bprob + BBprob));
            SFReshaping_down = readerReshaping.eval_auto_bounds("down_hfstats2", BTagEntry::FLAV_C, abs(Eta), Pt, (Bprob + BBprob));
            SFLoose = reader.eval_auto_bounds("central", BTagEntry::FLAV_C, abs(Eta), Pt, (Bprob + BBprob));
            SFMedium = readerMediumOP.eval_auto_bounds("central", BTagEntry::FLAV_C, abs(Eta), Pt, (Bprob + BBprob));
            SFTight = readerTightOP.eval_auto_bounds("central", BTagEntry::FLAV_C, abs(Eta), Pt, (Bprob + BBprob));
        } else {
            SFReshaping = readerReshaping.eval_auto_bounds("central", BTagEntry::FLAV_UDSG, abs(Eta), Pt, (Bprob + BBprob));
            SFReshaping_up = readerReshaping.eval_auto_bounds("up_hfstats2", BTagEntry::FLAV_UDSG, abs(Eta), Pt, (Bprob + BBprob));
            SFReshaping_down = readerReshaping.eval_auto_bounds("down_hfstats2", BTagEntry::FLAV_UDSG, abs(Eta), Pt, (Bprob + BBprob));
            SFLoose = reader.eval_auto_bounds("central", BTagEntry::FLAV_UDSG, abs(Eta), Pt, (Bprob + BBprob));
            SFMedium = readerMediumOP.eval_auto_bounds("central", BTagEntry::FLAV_UDSG, abs(Eta), Pt, (Bprob + BBprob));
            SFTight = readerTightOP.eval_auto_bounds("central", BTagEntry::FLAV_UDSG, abs(Eta), Pt, (Bprob + BBprob));
        }
...
void SortJets(std::vector<BJetCandidate>& items) {
  ...
  do {
    ...
    for (unsigned i = 1; i < items.size(); i++) {
      ...
      if ((items[i-1].Bprob + items[i-1].BBprob) < (items[i].Bprob + items[i].BBprob)) {
        std::swap(items[i-1], items[i]);
        swapped = true;
      } else if ((items[i-1].Bprob + items[i-1].BBprob) == (items[i].Bprob + items[i].BBprob)
