...
private:
    ...
    float tau_SFvsDM_MVA;
    float tau_SFvsDM_MVA_Up;
    float tau_SFvsDM_MVA_Down;
...
void TTbarTauLepton::beginJob() {
    ...
    if (isMC) {
        ...
        tree->Branch("tau_SFvsDM_MVA", &tau_SFvsDM_MVA, "tau_SFvsDM_MVA/F");
        tree->Branch("tau_SFvsDM_MVA_Up", &tau_SFvsDM_MVA_Up, "tau_SFvsDM_MVA_Up/F");
        tree->Branch("tau_SFvsDM_MVA_Down", &tau_SFvsDM_MVA_Down, "tau_SFvsDM_MVA_Down/F");
...
void TTbarTauLepton::TriggerMatching (const edm::Event& iEvent) {
    ...
    tau_TriggerMatch_selected = TriggerMatchingFunc(triggerObjects, triggerResults, triggerNames,
                                                    tau_pt, tau_eta, tau_phi,
                                                    trigNamesSelected, monitoringHLTmatching);
...
bool TTbarTauLepton::AddLepton (const edm::Event& event) {
    ...
    if (electrons.isValid() && !(*electrons).empty()) {
        ...
        for (auto& electron: *electrons) {
            ...
            if (monitoringLeptons) {
                if (privateMC_v1) std::cout << "MVA based loose ID = " << electron.electronID("cutBasedElectronID-Spring15-25ns-V1-standalone-loose") << std::endl;
                else std::cout << "MVA based loose ID = " << electron.electronID("mvaEleID-Fall17-noIso-V1-wpLoose") << std::endl;
            }
            // cutBasedElectronID-Spring15-25ns-V1-standalone-veto
            if (privateMC_v1) {
                cut(electron.electronID("cutBasedElectronID-Spring15-25ns-V1-standalone-loose"));
            } else {
                cut(electron.electronID("mvaEleID-Fall17-noIso-V1-wpLoose")); // loose for a start (cutbased id is also availible)
            }
            //
            if (monitoringLeptons) {
...
void TTbarTauLepton::GetTauSF (const edm::Event& iEvent) {
    ...
    tau_SFvsDM_Down     = 1.0;
    tau_SFvsDM_MVA      = 1.0;
    tau_SFvsDM_MVA_Up   = 1.0;
    tau_SFvsDM_MVA_Down = 1.0;
    tau_antiEleSF       = 1.0;
    ...
    std::string Year  = "2017ReReco";
    std::string TauID = "MVAoldDM2017v2";
    ...
    tau_SFvsDM      = sftoolDM->getSFvsDM(tau_pt, tau_dm, genmatch);
    tau_SFvsDM_Up   = sftoolDM->getSFvsDM(tau_pt, tau_dm, genmatch, "Up");
    tau_SFvsDM_Down = sftoolDM->getSFvsDM(tau_pt, tau_dm, genmatch, "Down");
    //
    tau_SFvsDM_MVA      = sftoolDM->getSFvsDM(tau_pt, tau_MVADM2017_v1, genmatch);
    tau_SFvsDM_MVA_Up   = sftoolDM->getSFvsDM(tau_pt, tau_MVADM2017_v1, genmatch, "Up");
    tau_SFvsDM_MVA_Down = sftoolDM->getSFvsDM(tau_pt, tau_MVADM2017_v1, genmatch, "Down");

    TauIDSFTool* antiEleSFTool = new TauIDSFTool(Year,"antiEleMVA6", "Tight");
    TauIDSFTool* antiMuSFTool  = new TauIDSFTool(Year,"antiMu3", "Tight");
    ...
    if (monitoringTau) {
        ...
        << "tau vs MVA DM   = " << tau_SFvsDM_MVA << std::endl
        << "andtiEle vs Eta = " << tau_antiEleSF << std::endl
        << "andtiMu vs Eta  = " << tau_antiMuSF << std::endl;
    }
}
